...
public:
  ...
  void setColor(uint8_t r, uint8_t g, uint8_t b, uint8_t brightness); // set color directly
  void light(uint8_t level); // set to white and return to last effect when set to 0
  void lightTimer(uint16_t sec);
  void alert(uint16_t sec);
...
private:
  ...
  uint8_t m_lightRevert;
  uint16_t m_lightTimer;
  uint16_t m_alertTimer;
  CRGB m_Color[4];
  CRGB m_leds[LED_CNT];
